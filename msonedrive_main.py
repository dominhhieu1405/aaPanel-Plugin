...
# coding: utf-8
# +-------------------------------------------------------------------
# | Microsoft's cloud network disk service
# +-------------------------------------------------------------------
...
class OSClient(object):
    ...
    def resumable_upload(self, *arg, **kwargs):
        """断点续传子类实现"""
        raise RuntimeError("Upload operation not supported!")

    def delete_object_by_os(self, object_name):
        """OS客户端实现删除操作"""
        raise RuntimeError("The file cannot be deleted!")
    ...
    def upload_file(self, file_name, data_type, *args, **kwargs):
        ...
        try:
            ...
            if not file_name or not data_type:
                _error_msg = "File parameter error."
                print(_error_msg)
            ...
            if self.error_msg:
                self.error_msg += r"\n"
            self.error_msg += "File upload encountered an error: {}".format(str(e))
            return False
    ...
    def delete_object(self, object_name, retries=2):
        ...
        try:
            ...
            print("File deletion exception: ")
            print(e)

        # 重试
        if retries > 0:
            print("Retrying to delete file {}..".format(object_name))
    ...
    def execute_by_comandline(self, args):
        ...
        try:
            ...
            #     data = client.get_files(args[2]);
                ...
                if result:
                    print("File {} deleted successfully. ".format(args[2]))
                else:
                    print("File {} deletion failed! ".format(args[2]))
            else:
                data = 'EERROR: Incorrect parameter!';
            if data:
                print()
...
class OneDriveClient(OSClient, object):
    ...
    def store_user(self):
        ...
        if user:
            ...
            raise RuntimeError("Unable to retrieve user information.")
    ...
    def get_token(self):
        ...
        if token is None:
            raise UnauthorizedError("Authorization information not received. Please authorize the plugin first and try again.")
    ...
    def get_user_from_ms(self):
        ...
        try:
            ...
            self.clear_auth()
            if DEBUG:
                print("User authorization has expired. Please reauthorize.")
        return None
    ...
    def resumable_upload(self,
        ...
        try:
            ...
            if part_size % 320 != 0:
                if DEBUG:
                    print("The part size must be a multiple of 320.")
                return False
            ...
            print("|- Uploading to {}...".format(object_name))
            ...
            if False:
                ...
                if response.status_code == 200:
                    ...
                    if DEBUG:
                        print("Upload session established.")
                    ...
                    for i in range(parts):
                        if DEBUG:
                            if i == parts - 1:
                                ...
                                num = "The {}".format(i + 1)
                            print("Uploading {} part...".format(num))
                        ...
                        if sub_response.status_code in expected_status_code:
                            if DEBUG:
                                print("Response status code: {}, "
                                      "Bytes {}-{} uploaded successfully.".format(
                            ...
                            if sub_response.status_code in [200, 201]:
                                if DEBUG:
                                    print("File {} uploaded successfully.".format(object_name))
                                return True
                        else:
                            print(sub_response.status_code)
                            print(sub_response.text)
                            _error_msg = "Bytes {}-{} chunk upload failed.".format(
                                upload_range_start,
                                upload_range_end
                            )
                            if self.error_msg:
            ...
            print("File upload encountered an error: ")
            print(e)

            if self.error_msg:
                self.error_msg += r"\n"
            self.error_msg += "File {} upload encountered an error: {}".format(object_name, str(e))

            try:
                if upload_url:
                    if DEBUG:
                        print("Clearing upload session.")
        ...
        if retries > 0:
            ...
            if self.error_msg:
                self.error_msg += r"\n"
            self.error_msg += "File {} upload failed.".format(object_name)
        return False
    ...
    def create_dir(self, dir_name):
        ...
        if re.search(onedrive_business_reserved, dir_name) \
            ...
            if DEBUG:
                print("The folder name contains illegal characters.")
            return False
        ...
        if response.status_code in [201, 409]:
            if DEBUG:
                if response.status_code == 409:
                    print("Directory: {} already exists. ".format(sub_folder))
            return True
        else:
            if DEBUG:
                print("Directory: {} creation failed: ".format(sub_folder))
                print(response.status_code)
                print(response.text)
        return False
    ...
    def download_file(self,
        ...
        try:
            ...
            print("An error occurred while downloading file {}: ".format(object_name) + str(e))
    ...
    def generate_download_url(self, object_name, expires=60 * 60):
        ...
        try:
            ...
            raise RuntimeError("An error occurred while querying the download link: " + str(e))
    ...
    def delete_object_by_os(self, object_name):
        ...
        if obj is None:
            if DEBUG:
                print("The object does not exist, so the deletion operation was not performed. ")
            return True
        if self.is_folder(obj):
            child_count = obj["folder"]["childCount"]
            if child_count > 0:
                if DEBUG:
                    print("The folder cannot be deleted as it is not empty.")
                return False
        ...
        if response.status_code == 204:
            if DEBUG:
                print("Object: {} has been deleted.".format(object_name))
            return True
        return False
...
class msonedrive_main:
    ...
    def set_config(self, get):
        ...
        try:
            if self.client.set_config(conf_obj):
                return public.returnMsg(True, "Configuration modification successful!")
        except:
            pass
        return public.returnMsg(False, "Configuration modification failed!")
    ...
    def create_dir(self, get):
        try:
            ...
            if self.client.create_dir(path):
                return public.returnMsg(True, 'Creation successful!');
            else:
                return public.returnMsg(False, "Creation failed!")
        except KeyError as e:
            return public.returnMsg(False, "Directory creation failed: " + str(e))
    ...
    def get_list(self, get):
        try:
            ...
            return public.returnMsg(False, "Authorization expired.")
        except Exception:
            return public.returnMsg(False, "Failed to retrieve the list!")

    def clear_auth(self, get):
        try:
            self.client.clear_auth()
            return public.returnMsg(True, "Local authorization has been revoked.")
        except:
            return public.returnMsg(False, "Authorization revocation failed!")
    ...
    def set_auth_url(self, get):
        try:
            ...
            return public.returnMsg(True, "Authorization successful!")
        except Exception:
            pass
        return public.returnMsg(False, "Authorization failed!")
    ...
    def delete_file(self, get):
        try:
            ...
            if file_name[-1] == "/":
                return public.returnMsg(False, "Directory deletion is currently not supported.")

            if file_name[:1] == "/":
                file_name = file_name[1:]
            if self.client.delete_object(file_name):
                return public.returnMsg(True, 'Deletion successful.')
            return public.returnMsg(False, 'File {} deletion failed!'.format(file_name))
        except:
    ...
    def get_lib(self):
        ...
        info = {
            ...
            "type": "Scheduled task",
            "ps": "Microsoft's cloud network disk service.",
            "status": 'false',
            "opt": "msonedrive",
            "module": "msonedrive",
            "script": "msonedrive",
            "help": "https://www.bt.cn/bbs/thread-54124-1-1.html",
            "backup_path": "Backup storage path, the default is /bt_backup",
            "check": [
    ...
    def change_user_type(self, data):
        ...
        return public.returnMsg(False, "Failed to modify account type!")
